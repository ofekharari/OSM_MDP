...
public class MyProbTable2 implements ProbTable {
	...
	public MyProbTable2(int len , int high) {
		this.len = len;
		this.high = high;
	...
	public double getProbByActionAndStates(Action action, State srcState, State dstState) {
		...
		if(location == Location.REGULAR) {
			...
			if(relation == Relation.SELF) {
				if(actionRelation.get(action) == Relation.NEIGHBOR_D) return 0.4;
				return 0.2;
			}
		}
		else if(location == Location.EDGE_U) {
			if(relation == Relation.SELF) {
				if(actionRelation.get(action) == Relation.NEIGHBOR_U) return 0.4;
				return 0.2;
			}
		}
		else if(location == Location.EDGE_L) {
			if(relation == Relation.SELF) {
				if(actionRelation.get(action) == Relation.NEIGHBOR_L) return 0.4;
				return 0.2;
			}
		}
		else if(location == Location.EDGE_R) {
			if(relation == Relation.SELF) {
				if(actionRelation.get(action) == Relation.NEIGHBOR_R) return 0.4;
				return 0.2;
			}
		}
			...
			if(relation == Relation.SELF) {
				if(actionRelation.get(action) == Relation.NEIGHBOR_L || 
				   actionRelation.get(action) == Relation.NEIGHBOR_D) return 0.6;
				return 0.4;
			}
		}
			...
			if(relation == Relation.SELF) {
				if(actionRelation.get(action) == Relation.NEIGHBOR_L || 
				   actionRelation.get(action) == Relation.NEIGHBOR_U) return 0.6;
				return 0.4;
			}
		}
		...
		if(actionRelation.get(action) == relation) return 0.4;
		return 0.2;
	}
